<?php
include __DIR__ . '/header.php';
?>

<div class="container-wrapper">
    <h1>Haarlem Festival</h1>

    <div class="landingimage">
        <img src="/img/landingimage.png" alt="">
    </div>



    <div class="festival-banner">
        <a href="/?pageid=2">
            <div class="festival-promotion-header" style='background-image: url(/img/historyPromotion.png);'>
                <h2>Stroll Through History</h2>
            </div>
        </a>

        <div class="banner-content">
            <!-- for every event / artist make this-->
            <div class="individual-event">
                <img src="/img/bavoKerkBanner.jpg" alt="">
                <p>Church of st Bravo</p>
            </div>

            <div class="individual-event">
                <img src="/img/groteMarktBanner.png" alt="">
                <p>Grote Markt</p>
            </div>

            <div class="individual-event">
                <img src="/img/deHallenBanner.jpg" alt="">
                <p>De Hallen</p>
            </div>

            <div class="individual-event">
                <img src="/img/proveniershofBanner.png" alt="">
                <p>Proveniershof</p>
            </div>

            <div class="individual-event">
                <img src="/img/jopenkerkBanner.png" alt="">
                <p>Jopenkerk</p>
            </div>

            <div class="individual-event">
                <img src="/img/waalsekerkBanner.png" alt="">
                <p>Waalse Kerk Haarlem</p>
            </div>

            <div class="individual-event">
                <img src="/img/molenBanner.png" alt="">
                <p>Molen de Adriaan</p>
            </div>

            <div class="individual-event">
                <img src="/img/amsterdamseBanner.png" alt="">
                <p>Amsterdamse Poort</p>
            </div>

            <div class="individual-event">
                <img src="/img/hofBanner.png" alt="">
                <p>Hof van Bakenes</p>
            </div>
        </div>
    </div>

    <div class="festival-banner">
        <a href="/?pageid=3">
            <div class="festival-promotion-header" style="background-image: url(/img/dancePromotion.png);">
                <h2>Dance! the night away</h2>
            </div>
        </a>

        <div class="banner-content">
            <!-- for every event / artist make this-->
            <div class="individual-event">
                <img src="/img/nickiBanner.png" alt="">
                <p>Nicki Romero</p>
            </div>

            <div class="individual-event">
                <img src="/img/tiestoBanner.png" alt="">
                <p>Tiesto</p>
            </div>

            <div class="individual-event">
                <img src="/img/hardwellBanner.png" alt="">
                <p>Hardwell</p>
            </div>

            <div class="individual-event">
                <img src="/img/afrojackBanner.png" alt="">
                <p>Afrojack</p>
            </div>

            <div class="individual-event">
                <img src="/img/arminBanner.png" alt="">
                <p>Armin Van Buuren</p>
            </div>

            <div class="individual-event">
                <img src="/img/martinBanner.png" alt="">
                <p>Martin Garrix</p>
            </div>
        </div>
    </div>

    <div class="festival-banner">
        <a href="/?pageid=4">
            <div class="festival-promotion-header" style="background-image: url(/img/jazzPromotion.jpg);">
                <h2>Jazz! and chill</h2>
            </div>
        </a>
        <div class="banner-content">
            <!-- for every event / artist make this-->
            <div class="individual-event">
                <img src="/img/gumboBanner.png" alt="">
                <p>Gumbo Kings</p>
            </div>

            <div class="individual-event">
                <img src="/img/evolveBanner.png" alt="">
                <p>Evolve</p>
            </div>

            <div class="individual-event">
                <img src="/img/ntjamBanner.png" alt="">
                <p>Ntjam Rose</p>
            </div>

            <div class="individual-event">
                <img src="/img/wickedBanner.png" alt="">
                <p>Wicked jazz sounds</p>
            </div>

            <div class="individual-event">
                <img src="/img/jonnaBanner.png" alt="">
                <p>Jonna Frazer</p>
            </div>

            <div class="individual-event">
                <img src="/img/tomBanner.png" alt="">
                <p>Tom thomsom assemble</p>
            </div>

            <div class="individual-event">
                <img src="/img/foxthemayors.jpg" alt="">
                <p>Fox & The Mayors</p>
            </div>

            <div class="individual-event">
                <img src="/img/unclesue.jpg" alt="">
                <p>Uncle Sue</p>
            </div>

            <div class="individual-event">
                <img src="/img/krisallen.jpeg" alt="">
                <p>Kris Allen</p>
            </div>

            <div class="individual-event">
                <img src="/img/mylessanki.jpg" alt="">
                <p>Myles Sanko</p>
            </div>

            <div class="individual-event">
                <img src="/img/soulsix.jpeg" alt="">
                <p>Soul Six</p>
            </div>

            <div class="individual-event">
                <img src="/img/ruis.jpg" alt="">
                <p>Ruis Soundsystem</p>
            </div>

            <div class="individual-event">
                <img src="/img/familyxl.jpeg" alt="">
                <p>The Family XL</p>
            </div>

            <div class="individual-event">
                <img src="/img/garedenord.jpg" alt="">
                <p>Gare Du Nord</p>
            </div>

            <div class="individual-event">
                <img src="/img/rilan.jpeg" alt="">
                <p>Rilan and the bombardiers</p>
            </div>

            <div class="individual-event">
                <img src="/img/han bennink.jpg" alt="">
                <p>Han Bennink</p>
            </div>

            <div class="individual-event">
                <img src="/img/nordanians.jpg" alt="">
                <p>The Nordanians</p>
            </div>

            <div class="individual-event">
                <img src="/img/lilith.jpg" alt="">
                <p>Lilith Merlot</p>
            </div>
        </div>
    </div>

    <div class="festival-banner">
        <a href="/?pageid=5">
            <div class="festival-promotion-header" style="background-image: url(/img/yummyPromotion.png);">
                <h2>Yummy! Foodies </h2>
            </div>
        </a>
        <div class="banner-content">
            <!-- for every event / artist make this-->
            <div class="individual-event">
                <img src="/img/cafederoemer.jpg" alt="">
                <p>Cafe de roemer</p>
            </div>

            <div class="individual-event">
                <img src="/img/ratatoulle.jpg" alt="">
                <p>Ratatouille</p>
            </div>

            <div class="individual-event">
                <img src="/img/ML.jpg" alt="">
                <p>ML Restaurant</p>
            </div>

            <div class="individual-event">
                <img src="/img/fris.jpg" alt="">
                <p>Fris Restaurant</p>
            </div>

            <div class="individual-event">
                <img src="/img/specktackel.jpg" alt="">
                <p>Specktacel</p>
            </div>

            <div class="individual-event">
                <img src="/img/cafebrinkman.jpg" alt="">
                <p>Grand cafe Brinkman</p>
            </div>

            <div class="individual-event">
                <img src="/img/urbanfrenchy.jpg" alt="">
                <p>Urban Frenchy Bistro Toujours</p>
            </div>
        </div>
    </div>
</div>
<?php
include __DIR__ . '/footer.php';
?>